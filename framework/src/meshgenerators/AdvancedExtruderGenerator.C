        ...
        else
        {
          ...
          if (_end_radial_extent && start_node_radius > libMesh::TOLERANCE)
          {
            ...
            // The radial expansion has to be performed in the rotated frame
              ...
              node_to_extrusion_axis = *node + current_step - (reference_point + _direction * cumulative_step_size);
            // when extruding in a straight line with no twisting, we can use the original radial direction
            else
            ...
            const auto radial_ratio_m1 =
                AdvancedExtruderGenerator::radialExpansionRatio(tm1);
            const auto radial_ratio =
                AdvancedExtruderGenerator::radialExpansionRatio(t);

            // Compute change in step
            current_step +=
                (start_radial_extent * (radial_ratio_m1 - radial_ratio) + (radial_ratio - radial_ratio_m1) * _end_radial_extent) *
          ...
          if (!MooseUtils::absoluteFuzzyEqual(_twist_pitch, 0.))
          {
            ...
            // If we normalize twist, we must multiply by 2 * sin(libMesh::pi * step_size / _twist_pitch)
            // if (!MooseUtils::absoluteFuzzyEqual(twist.norm(), .0))
            //   twist /= twist.norm();
            ...
            else
            {
              ...
              prev_extrusion_axis_at_elevation = reference_point + _direction * (cumulative_step_size - step_size);
            }
