...
DeleteElementsNearMeshGenerator::validParams()
{
  ...
  params.addClassDescription("Removes elements lying 'near' another mesh. The proximity is examined by the distance from the element's centroid to the faces of elements of the 'proximity_mesh'");
  params.addRequiredParam<MeshGeneratorName>("proximity_mesh", "Mesh providing the deletion criterion");
  params.addRequiredRangeCheckedParam<Real>("distance", "distance>0", "The distance from the centroid of elements in the 'input' mesh to elements in the 'proximity_mesh' under which they are marked for deletion");
  params.addRequiredParam<MooseEnum>("side_order", SetupQuadratureAction::getQuadratureOrderEnum(), "Order of the face quadrature used to find the nearest face in the 'proximity_mesh'");
...
DeleteElementsNearMeshGenerator::generate()
{
  ...
  // NOTE: side Qps are just one option. We could have done nodes, side centroids (~ side Qp at order 0)
  std::vector<Point> all_side_qps;
  const auto order = getParam<MooseEnum>("side_order");
  all_side_qps.reserve(_proximity_mesh->n_elem() * _proximity_mesh->spatial_dimension() * 2 * order);
