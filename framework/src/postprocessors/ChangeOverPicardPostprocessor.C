...
// moose includes
#include "ChangeOverPicardPostprocessor.h"
#include "Transient.h"

registerMooseObject("MooseApp", ChangeOverPicardPostprocessor);

defineLegacyParams(ChangeOverPicardPostprocessor);

InputParameters
ChangeOverPicardPostprocessor::validParams()
{
  ...
  params.addClassDescription("Computes the change or relative change in a post-processor value "
                             "over a single or multiple Picard iterations");

  return params;
}

ChangeOverPicardPostprocessor::ChangeOverPicardPostprocessor(const InputParameters & parameters)
    ...
    _t_step_old(-1)
{
  if (_change_with_respect_to_initial)
  {
    ...
    if (!_execute_enum.contains(EXEC_INITIAL))
      mooseError("When 'change_with_respect_to_initial' is specified to be true, 'execute_on' for "
                 "the ChangeOverPicardPostprocessor ('" +
                 name() + "') must include 'initial'");
  }
}

void
ChangeOverPicardPostprocessor::initialize()
{
}

void
ChangeOverPicardPostprocessor::execute()
{
}

Real
ChangeOverPicardPostprocessor::getValue()
{
