  ...
  switch (_current_direction)
  {
    ...
    case FROM_MULTIAPP:
    {
      ...
      for (unsigned int i = 0; i < _from_multi_app->numGlobalApps(); i++)
      {
        ...
        Point app_position = _from_multi_app->position(i);
        BoundingBox app_box = _from_multi_app->getBoundingBox(i, _displaced_source_mesh);
        const UserObject & user_object = _from_multi_app->appUserObjectBase(i, _user_object_name);
          ...
          for (auto & node : to_mesh->getMesh().node_ptr_range())
          {
            ...
            if (node->n_dofs(to_sys_num, to_var_num) > 0) // If this variable has dofs at this node
            {
              ...
              if (_skip_bbox_check || app_box.contains_point(*node))
              {
                ...
                Real from_value = 0;
                {
                  Moose::ScopedCommSwapper swapper(_from_multi_app->comm());
