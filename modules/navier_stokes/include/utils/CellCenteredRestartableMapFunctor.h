 ...
 */
template <typename T, typename MapKey, typename MapValue>
class CellCenteredRestartableMapFunctor : public Moose::FunctorBase<T>, public Restartable
{
public:
   ...
   */
  CellCenteredRestartableMapFunctor(const MooseMesh & mesh,
  ...
  /// Return the map, notably for inserting new key/values in it
  std::unordered_map<MapKey, MapValue> & getMap();

private:
  /// The map data held by the functor
  std::unordered_map<MapKey, MapValue> & _map;
...
template <typename T, typename MapKey, typename MapValue>
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::CellCenteredRestartableMapFunctor(
    const MooseMesh & mesh, const std::string & name, const bool extrapolated_boundary)
  : Moose::FunctorBase<T>(name),
    Restartable(mesh.getMooseApp(),
              name,
              "Functors",
              0),
    _map(this->template declareRestartableData<std::unordered_map<MapKey, MapValue>>("name")),
    _mesh(mesh),
...
template <typename T, typename MapKey, typename MapValue>
bool
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::isExtrapolatedBoundaryFace(
    const FaceInfo & fi, const Elem *, const StateArg &) const
{
...
template <typename T, typename MapKey, typename MapValue>
bool
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::hasBlocks(const Elem * const elem) const
{
  if (!elem)
    return false;
...
template <typename T, typename MapKey, typename MapValue>
bool
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::hasBlocks(const SubdomainID sub_id) const
{
  return _sub_ids.empty() || _sub_ids.count(sub_id);
}

template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::ValueType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluate(const ElemArg & elem_arg,
                                                                 const StateArg &) const
{
  ...
  catch (libMesh::LogicError &)
  {
    if (!_sub_ids.empty() && !_sub_ids.count(elem->subdomain_id()))
      mooseError("Attempted to evaluate CellCenteredRestartableMapFunctor '",
                 this->functorName(),
      ...
      mooseError(
          "Attempted access into CellCenteredRestartableMapFunctor '",
          this->functorName(),
          "' with a key that does not yet exist in the map. Make sure to fill your "
          "CellCenteredRestartableMapFunctor for all elements you will attempt to access later.");
  }
}

template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::ValueType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluate(const ElemPointArg & elem_point,
...
template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::ValueType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluate(const FaceArg & face,
                                                                 const StateArg & state) const
{
...
template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::ValueType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluate(const ElemQpArg &,
                                                                 const StateArg &) const
{
...
template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::ValueType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluate(const ElemSideQpArg &,
                                                                 const StateArg &) const
{
  mooseError("not implemented");
}
...
template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::ValueType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluate(const NodeArg &,
                                                                 const StateArg &) const
{
  mooseError("not implemented");
}
...
template <typename T, typename MapKey, typename MapValue>
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::GradientType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluateGradient(
    const ElemArg & elem_arg, const StateArg & state) const
{
...
typename CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::GradientType
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::evaluateGradient(
    const FaceArg & face, const StateArg & state) const
{
  return Moose::FV::greenGaussGradient(face, state, *this, _extrapolated_boundary, _mesh);
}

template <typename T, typename MapKey, typename MapValue>
std::unordered_map<MapKey, MapValue> &
CellCenteredRestartableMapFunctor<T, MapKey, MapValue>::getMap()
{
  return _map;
}
