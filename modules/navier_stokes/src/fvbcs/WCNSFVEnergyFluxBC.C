...
WCNSFVEnergyFluxBC::WCNSFVEnergyFluxBC(const InputParameters & params)
  : FVFluxBC(params),
    ...
    _temperature_pp(isParamValid("temperature_pp") ? &getPostprocessorValue("temperature_pp") : nullptr),
    ...
    _cp(isParamValid(NS::cp) ? &getFunctor<ADReal>(NS::cp) : nullptr)
{
  ...
  if (_energy_pp && (_velocity_pp || _mdot_pp || _temperature_pp))
    mooseWarning("If setting the temperature directly, no need for inlet velocity, mass flow or energy");
  ...
  if (!_energy_pp) {
    ...
    if (!_velocity_pp && !_mdot_pp)
      mooseError("If not providing the inlet energy flow rate, the inlet velocity or mass flow should be provided");
    if (_velocity_pp && (!_rho || !_cp))
      mooseError("If providing the inlet velocity, the density, the area and the specific heat capacity should be provided as well");
    if (_mdot_pp && (!_cp || !_area_pp))
      mooseError("If providing the inlet mass flow rate, the inlet specific heat capacity and flow area should be provided as well");
  }
  else
    if (!_area_pp)
      paramError("energy_pp", "If supplying the energy flow rate, the flow area should be provided as well");
}
...
WCNSFVEnergyFluxBC::computeQpResidual() {
  ...
  if (_energy_pp)
    ...
    return -_scaling_factor * (*_rho)(singleSidedFaceArg()) * *_velocity_pp * (*_cp)(singleSidedFaceArg()) * *_temperature_pp;
  else
    return -_scaling_factor * *_mdot_pp / *_area_pp * (*_cp)(singleSidedFaceArg()) * *_temperature_pp;
}
