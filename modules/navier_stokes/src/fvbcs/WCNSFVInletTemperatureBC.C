    ...
    _cp(isParamValid(NS::cp) ? &getFunctor<ADReal>(NS::cp) : nullptr)
{
  ...
  if (!_temperature_pp) {
    ...
    if (!_velocity_pp && !_mdot_pp)
      mooseError("If not providing the inlet temperature, the inlet velocity or mass flow should be provided");
    ...
    if (_mdot_pp && !_cp)
      mooseError("If providing the inlet mass flow rate, the inlet specific heat capacity should be provided as well");
  }
  else
    if (params.isParamSetByUser("scaling_factor"))
      paramError("scaling_factor", "The scaling factor is meant to adjust for a different area or "
