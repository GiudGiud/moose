...
VolumetricFlowRate::validParams()
{
  ...
  params.addCoupledVar("advected_variable", "The advected variable quantity of which to study the flow");
  params.addParam<MaterialPropertyName>("advected_mat_prop",
...
VolumetricFlowRate::computeQpIntegral()
{
  ...
  if (_fv)
  {
    ...
    mooseAssert(!fi->isBoundary(), "VolumetricFlowRate should only be used inside the domain");
