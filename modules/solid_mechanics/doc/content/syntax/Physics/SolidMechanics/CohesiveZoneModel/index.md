...
The `CohesiveZoneModel` action automatically adds the proper `ComputeDisplacementJump`, `ComputeGlobalTraction`, `CZMInterfaceKernel` based on the `kinematic_type` parameter value (see inputs).
The flowchart below summarizes the flow of information of the cohesive zone modeling frameworks, and highlights the  objects automatically added by the `CohesizeZoneModel` action.

!media media/solid_mechanics/CZMMasterAction.png style=width:100%;

!alert note
Even when using the `CohesiveZoneModel` action it is the responsibility of the user to add the appropriate `ComputeLocalTraction` constitutive model and `BreakMeshByBlockGenerator` in the input file.
...
As mentioned in previous section, the `CohesiveZoneModel` action adds the appropriate objects depending on the selected kinematic formulation.
...
The following example show how to use the `CohesiveZoneModel` action.

!listing modules/solid_mechanics/test/tests/cohesive_zone_model/stretch_rotate_large_deformation.i block=Physics/SolidMechanics/CohesiveZoneModel

If necessary, multiple instances of the `CohesiveZoneModel` action can be added, for instance when different material properties `base_name` are needed for different boundaries. The `base_name` parameter used in the action should also be provided to the associated materials.
...
!listing modules/solid_mechanics/test/tests/cohesive_zone_model/czm_multiple_action_and_materials.i start=[Physics/SolidMechanics/CohesiveZoneModel] end=[Modules] include-end=false

!syntax parameters /Physics/SolidMechanics/CohesiveZoneModel/CohesiveZoneAction
!syntax inputs /Physics/SolidMechanics/CohesiveZoneModel/CohesiveZoneAction

## Associated Actions

!syntax list /Physics/SolidMechanics/CohesiveZoneModel objects=True actions=False subsystems=False

!syntax list /Physics/SolidMechanics/CohesiveZoneModel objects=False actions=False subsystems=True

!syntax list /Physics/SolidMechanics/CohesiveZoneModel objects=False actions=True subsystems=False
