...
ErrorMaterial::validParams()
{
  ...
  params.addClassDescription("Test Material that throws MooseErrors at specified locations for testing purposes. We want to check that the materials are not computed in certain locations, as an optimization and a sanity check on the correct execution of the restriction logic");
  params.addParam<std::vector<BoundaryName>>("sidesets_to_error_on", {}, "List of sidesets to error at if the material is executed on that face");
...
ErrorMaterial::computeQpProperties()
{
  ...
  if (_bnd)
  {
    ...
    std::set_intersection(boundary_ids.begin(),boundary_ids.end(),
